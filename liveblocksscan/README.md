...
- **Исходящий перевод с адреса-оператора** (`hacker`, `suspicious`) —
  движение украденного: `SUSPICIOUS` + `F1_DOWNSTREAM_TRANSFER`,
  `victim=""`, `hacker=получатель`, `exposed=[отправитель]`. Получатель
  регистрируется как `suspicious`, дерево отчёта связывает выплату с
  отправителем. **Жертвы** (`hacked`, `drained`, `phishing`) как отправитель
  исходящих переводов не обрабатываются — деньги уже ушли при первом
  сведении, их оригинальные переводы нам не те, что попадают в ленту
  (исходящая активность жертвы — fl шум, не событие compromise).

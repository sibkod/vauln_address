#!/usr/bin/env python3
"""Polygon PoS: live-мониторинг блоков.
...
адреса через POST /api/check/bulk (chain=polygon -> evm) и постит находки
...
Запуск: python3 polygon.py [--api-url …] [--api-key …] [--interval 2.0]
Конфигурация также через env: VAULN_API_URL, ADMIN_API_KEY, POLYGON_RPC_URL
...
def main():
    args = parse_args("polygon", POLL_INTERVAL)
    endpoints = list(RPC_ENDPOINTS)
    env_eps = os.environ.get("POLYGON_RPC_URL")
    if env_eps:
        endpoints = [e.strip() for e in env_eps.split(",") if e.strip()]
    latest, transfers = make_evm_watcher("polygon", endpoints)
    run("polygon", CHAIN, args.interval, latest, transfers,
